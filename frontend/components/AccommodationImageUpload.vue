    ...
    <div class="accommodation-image-upload">
        ...
        <div class="upload-zone mt-6">
            ...
            <div v-if="newImages.length > 0" class="new-images-preview">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    <div v-for="(image, index) in newImages" :key="index" class="relative">
                        <img
                            :src="image.preview"
...
<script setup lang="ts">
    ...
    const uploadNewImages = async () => {
        ...
        try {
            ...
            $toast.error(
                (error as { data?: { message?: string } })?.data?.message || "Erreur lors de l'upload des images"
            );
        } finally {
            loading.value = false;
        }
    ...
    const deleteImage = async (imageId: number | string) => {
        ...
        try {
            ...
            $toast.error(
                (error as { data?: { message?: string } })?.data?.message || "Erreur lors de la suppression de l'image"
            );
        } finally {
            loading.value = false;
        }
