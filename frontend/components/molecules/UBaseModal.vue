...
<template>
    ...
    <transition name="modal">
        <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center" @click.self="close">
            <div class="relative bg-primary rounded-2xl shadow-xl max-w-lg w-full transform transition-all overflow-hidden">
                <component :is="backgroundComponent[background]" v-if="background !== 'empty'" :class="[backgroundSizeClasses[backgroundSize], 'scale-150 -z-10']"/>
