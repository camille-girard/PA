<script setup lang="ts">
    ...
    const daysInMonth = computed(() =>
        Array.from(
            { length: new Date(current.value.getFullYear(), current.value.getMonth() + 1, 0).getDate() },
            (_, i) => i + 1
        )
    );
    const blanks = computed(() =>
        Array(startsWithMonday.value ? (current.value.getDay() || 7) - 1 : current.value.getDay()).fill(0)
    );
    ...
    function goToToday() {
        const todayMonth = today.value.getMonth(),
            currentMonth = current.value.getMonth();
        direction.value = todayMonth > currentMonth ? 'next' : 'prev';
    ...
    function isToday(day: number) {
        ...
        return (
            day === t.getDate() &&
            current.value.getMonth() === t.getMonth() &&
            current.value.getFullYear() === t.getFullYear()
        );
    }
    ...
    function isDisabled(day: number) {
        ...
        return (props.minDate && date < props.minDate) || (props.maxDate && date > props.maxDate);
    }
    function handleDateInput() {
        const date = parseDate(dateInputValue.value);
        if (date) {
            selected.value = date;
            if (date.getMonth() !== current.value.getMonth() || date.getFullYear() !== current.value.getFullYear()) {
                direction.value = date > current.value ? 'next' : 'prev';
                current.value = new Date(date.getFullYear(), date.getMonth(), 1);
                emit('month-change', current.value);
            }
        } else if (dateInputValue.value) emit('invalid-date', dateInputValue.value);
    }
</script>
    ...
    <div class="min-w-82 w-fit bg-white rounded-2xl border border-gray-200 shadow-xl px-6 py-5 relative z-50">
        ...
        <div class="grid grid-cols-7">
            <p
                ...
                :key="day"
                class="text-gray-500 text-sm flex items-center justify-center size-10"
            >
                {{ day }}
            </p>
        </div>
