...
<template>
  <div class="space-y-8">
    <div class="bg-orange-100 rounded-2xl shadow-md p-6 flex flex-col items-center text-center">
      ...
      <div>
        <h3 class="text-xl font-bold text-gray-800">{{ host.name }}</h3>
        <p class="text-gray-600">{{ host.verified ? 'Hôte vérifié' : 'Non vérifié' }}</p>
      </div>

      <div class="mt-6 text-gray-700 space-y-1">
        <p>Notes : <span class="font-semibold">{{ host.note }}/5</span></p>
    ...
    <div class="border border-orange-300 rounded-2xl p-6 space-y-4">
      <h4 class="text-lg font-bold text-gray-800">Vérifications effectuées par {{ host.name }}</h4>
      <ul class="space-y-2">
        <li v-for="(verification, index) in host.verifications" :key="index" class="flex items-center gap-2">
    ...
    <div class="flex items-center gap-2">
      ...
      <a href="#" class="underline font-semibold text-black">Signaler ce profil</a>
    </div>
  </div>
</template>
