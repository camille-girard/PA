    ...
    <UCard>
      <template #header>
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center">
          ...
          <UBadge
            :color="owner?.isVerified ? 'success' : 'error'"
            variant="pill"
          >
    ...
    <BaseModal v-model="showModal" title="Disponibilités de l’hébergement">
        <template #default>
            ...
            <div v-if="selectedAccommodation?.bookings?.length">
                ...
                <li
                    v-for="booking in selectedAccommodation.bookings"
                    :key="booking.id"
                >
                    Du {{ new Date(booking.startDate).toLocaleDateString() }}
                    au {{ new Date(booking.endDate).toLocaleDateString() }} —
                    <strong>{{ booking.status }}</strong>
                </li>
                </ul>
            </div>
            <div v-else class="text-gray-500 text-sm">
                Aucun créneau réservé pour ce logement.
            </div>
            </div>
        </template>
        </BaseModal>
