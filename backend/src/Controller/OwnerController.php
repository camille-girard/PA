...
final class OwnerController extends AbstractController
{
    ...
    public function uploadAvatar(
        ...
        if (!$owner || $owner->isDeleted()) {
            return $this->json(['message' => 'Owner non trouvé'], Response::HTTP_NOT_FOUND);
        }
        ...
        try {
            ...
            if ($owner->getAvatar()) {
                $r2Service->deleteFile($owner->getAvatar());
            }

            // Upload new avatar
            $avatarUrl = $r2Service->uploadAvatar($file, $owner->getId());

            // Update owner
            $owner->setAvatar($avatarUrl);
            $this->entityManager->flush();

            // Return updated owner data
            $json = $serializer->serialize($owner, 'json', ['groups' => 'owner:read']);
